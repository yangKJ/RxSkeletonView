...
public class RxPickerViewDataSourceProxy
    ...
    private weak var dataSource: UIPickerViewDataSource? = pickerViewDataSourceNotSet

    /// For more information take a look at `DelegateProxyType`.
    public override func setForwardToDelegate(_ forwardToDelegate: UIPickerViewDataSource?, retainDelegate: Bool) {
        dataSource = forwardToDelegate ?? pickerViewDataSourceNotSet
...
extension RxPickerViewDataSourceProxy: UIPickerViewDataSource {
    ...
    public func numberOfComponents(in pickerView: UIPickerView) -> Int {
        (dataSource ?? pickerViewDataSourceNotSet).numberOfComponents(in: pickerView)
    }

    /// Required delegate method implementation.
    public func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        (dataSource ?? pickerViewDataSourceNotSet).pickerView(pickerView, numberOfRowsInComponent: component)
    }
}
