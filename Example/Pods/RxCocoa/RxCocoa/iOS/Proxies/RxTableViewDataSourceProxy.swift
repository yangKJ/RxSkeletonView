...
open class RxTableViewDataSourceProxy
    ...
    private weak var dataSource: UITableViewDataSource? = tableViewDataSourceNotSet

    /// For more information take a look at `DelegateProxyType`.
    open override func setForwardToDelegate(_ forwardToDelegate: UITableViewDataSource?, retainDelegate: Bool) {
        dataSource = forwardToDelegate  ?? tableViewDataSourceNotSet
...
extension RxTableViewDataSourceProxy: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        (dataSource ?? tableViewDataSourceNotSet).tableView(tableView, numberOfRowsInSection: section)
    }

    /// Required delegate method implementation.
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        (dataSource ?? tableViewDataSourceNotSet).tableView(tableView, cellForRowAt: indexPath)
    }
}
